...
import { analyzeResume, extractResumeTextWithFallback, fetchJobDescriptionContent } from './services/aiService';
import { EngineOrchestrator } from './services/engine/orchestrator';
...
import { restoreStateAfterPayment, clearPersistedState, saveStateBeforePayment } from './services/stateService';
import { parseAppRoute } from './services/routing';
    ...
    >
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="flex items-center gap-2 mt-4">
          ...
          />
          <span className="text-zinc-500 font-mono text-xs uppercase tracking-widest">Loading HireSchema...</span>
        </div>
      </div>
    </motion.div>
  );
...
export default function App() {
  ...
  // --- VIEWS ---
  const [view, setView] = useState<'landing' | 'dashboard' | 'legal' | 'roast' | 'blog' | 'feature' | 'pricing' | 'changelog' | 'success-stories' | 'what-is-hireschema'>(initialRoute.view);

  const [legalPage, setLegalPage] = useState<'privacy' | 'terms' | 'cookies' | null>(initialRoute.legalPage);
  ...
  const [featureId, setFeatureId] = useState<string | null>(initialRoute.featureId);
  ...
  useEffect(() => {
    if (isPaid) {
      const seen = localStorage.getItem('hasSeenReferral');
      if (!seen) {
        const timer = setTimeout(() => setShowReferral(true), 2500);
    ...
    const initRoute = async () => {
      ...
      if (paymentId) {
        ...
        try {
          ...
          const restoreUserState = async (): Promise<string | null> => {
            console.log('=== restoreUserState called ===');

            // First try to restore from persisted state (saved before redirect)
            const persistedState = await restoreStateAfterPayment();
            ...
            if (persistedState) {
              ...
              await clearPersistedState(); // Clean up after restore
          ...
          const markPaymentSuccess = async () => {
            const restoredId = await restoreUserState();
  ...
  useEffect(() => {
    const syncFromLocation = () => {
      const next = parseAppRoute(window.location.pathname, window.location.search);
      setView(next.view);
      setLegalPage(next.legalPage);
      setBlogSlug(next.blogSlug);
      setFeatureId(next.featureId);
    };
  ...
  const handleLandingStart = async (intent: 'landing' | 'scan' | 'optimize' | 'launch' | 'roast' | 'blog' | 'feature' | 'pricing' | 'changelog' | 'success-stories' | 'what-is-hireschema', file?: FileData, featureSlug?: string, jobDescription?: string) => {
    if (intent === 'landing') {
      setView('landing');
    ...
    // Set Job Description if provided
    setJobDescription(jobDescription || '');
    
    setAnalysisResult(null);
    
    // If both are provided, we can potentially set step to something else or just let the user verify
    // Since ResumeUploader in dashboard checks for resumeFile to show/hide step 1,
    // and checks for something else for step 2?
    // Let's check logic:
    // Step 1 visible if inputWizardStep !== 1
    // Step 2 visible if inputWizardStep !== 0
    // If we have file and JD, we ideally want to show Step 2 (to verify JD) or maybe just ready state.
    // If we have file, we usually set step 1 (which hides step 1 resume uploader and shows step 2).
    // Let's set step to 1 if file exists.
    
    setInputWizardStep(file ? 1 : 0);
    setDashboardView('scan');

    if (intent === 'scan' && file && (jobDescription || '').trim()) {
      setTimeout(() => {
  ...
  const handleRequestTab = (tab: 'analysis' | 'generator') => {
    if (tab === 'generator' && !isPaid) {
      setShowPaywall(true);
      return;
    }
    setResultTab(tab);
  ...
  const runAnalysis = async (fileToAnalyze: FileData, jdInput: string) => {
    ...
    try {
      ...
      let finalJobDescription = jdInput;
      if (jdInput.trim().startsWith('http')) {
        const url = jdInput.trim();
      ...
      const result = await analyzeResume(fileToAnalyze, finalJobDescription);
  ...
  const handleAnalysis = async () => {
    ...
    return runAnalysis(resumeFile, jobDescription);
  };
  ...
  return (
    <ErrorBoundary>
          ...
          {view === 'dashboard' && (
            <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex h-screen bg-black text-zinc-100 overflow-hidden font-sans w-full">
              ...
              <div className="flex-1 flex flex-col min-w-0 bg-zinc-950">
                {/* HEADER */}
                <header className="h-[80px] border-b border-white/15 bg-zinc-900/45 backdrop-blur-2xl shadow-[0_10px_30px_rgba(0,0,0,0.35)] flex items-center justify-between px-3 sm:px-6 shrink-0 safe-area-inset relative">
                  ...
                  <div className="max-w-7xl mx-auto w-full flex items-center justify-between relative">
                    <div className="cursor-pointer touch-target flex items-center scale-90 origin-left transition-transform duration-500 hover:scale-95" onClick={() => {
                      setView('landing');
                      window.history.pushState({}, '', '/');
                      window.scrollTo(0, 0);
                    }}><AnimatedLogo /></div>
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                      <button
                        ...
                        className="text-xs font-bold uppercase tracking-[0.2em] text-zinc-400 hover:text-white transition-all duration-300 relative group py-2 px-1"
                        title="View History"
                      >
                        <span>History</span>
                      ...
                      {isPaid && (
                        <div className="hidden sm:flex items-center gap-2 px-2.5 py-0.5 bg-orange-500/10 border border-orange-500/20 rounded-full backdrop-blur-sm">
                      ...
                      <a
                        ...
                        className="flex items-center gap-1.5 px-5 py-2.5 bg-white hover:bg-zinc-50 active:bg-zinc-200 text-black font-mono font-bold text-xs uppercase tracking-widest shadow-[3px_3px_0px_0px_#f97316] hover:shadow-none active:shadow-none hover:translate-x-[1.5px] hover:translate-y-[1.5px] active:translate-x-[1.5px] active:translate-y-[1.5px] transition-all rounded-sm cursor-pointer border-none touch-target"
                      >
                          ...
                          <>
                            <div className="flex justify-center mb-4">
                              <div className="bg-zinc-900 p-1 rounded-lg border border-zinc-800 flex gap-1">
                                <button onClick={() => handleRequestTab('analysis')} className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${resultTab === 'analysis' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>Diagnostic</button>
                                <button onClick={() => handleRequestTab('generator')} className={`px-3 py-1.5 rounded text-xs font-bold transition-all ${resultTab === 'generator' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>Editor</button>
                              </div>
                            </div>
                            <div className="flex-1 bg-zinc-900/30 border border-white/5 rounded-xl overflow-hidden">
                              {resultTab === 'analysis' || !isPaid ? (
                                  ...
                                  <ErrorBoundary>
                                    <Suspense fallback={<div className="flex items-center justify-center h-64"><LoadingIndicator size="md" message="Loading Analysis..." /></div>}>
                                      <AnalysisDashboard
                                        ...
                                        onNavigateTab={handleRequestTab}
                                ...
                                <ErrorBoundary>
                                  <Suspense fallback={<div className="flex items-center justify-center h-64"><LoadingIndicator size="md" message="Loading Editor..." /></div>}>
                                    <Editor
                                      ...
                                      isPaid={isPaid}
                                      onDownloadComplete={() => setShowReferral(true)}
      ...
      <AnimatePresence>
        {showPaywall && analysisResult && resumeFile && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={() => setShowPaywall(false)}
            className="fixed inset-0 z-[140] bg-black/85 backdrop-blur-md flex items-center justify-center p-6"
          >
            <motion.div
            ...
            >
              <button
                onClick={() => setShowPaywall(false)}
                className="absolute -top-3 -right-3 w-10 h-10 rounded-full bg-zinc-950 border border-white/10 text-white flex items-center justify-center hover:bg-zinc-900 transition-colors"
                title="Close"
              >
                <X className="w-4 h-4" />
              </button>
              <PaymentLock
                ...
                onBeforeRedirect={() => saveStateBeforePayment({
                  analysisId: selectedHistoryId || undefined,
                  resumeFile,
                  resumeText,
                  jobDescription,
