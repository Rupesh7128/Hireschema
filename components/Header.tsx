...
import { BookOpen, Zap, Flame, Menu, X } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({ onNavigate }) => {
  ...
  return (
    ...
    >
        <div className="cursor-pointer touch-target" onClick={() => { window.scrollTo(0, 0); onNavigate('landing'); }}>
            <AnimatedLogo />
        </div>
        
        {/* Desktop Nav */}
        <div className="hidden sm:flex items-center gap-2 sm:gap-6">
            <button onClick={() => onNavigate('blog')} className="flex items-center gap-2 text-xs font-bold text-zinc-400 hover:text-white transition-colors uppercase tracking-widest px-3 py-1.5 cursor-pointer touch-target">
                <BookOpen className="w-3 h-3" /> Blog
            </button>
            <button onClick={() => onNavigate('pricing')} className="flex items-center gap-2 text-xs font-bold text-zinc-400 hover:text-white transition-colors uppercase tracking-widest px-3 py-1.5 cursor-pointer touch-target">
                Pricing
            </button>
            <button onClick={() => onNavigate('roast')} className="flex items-center gap-2 text-xs font-bold text-orange-500 hover:text-orange-400 active:text-orange-300 transition-colors uppercase tracking-widest border border-orange-500/20 px-3 py-1.5 rounded bg-orange-500/5 hover:bg-orange-500/10 cursor-pointer touch-target">
        ...
        {isMobileMenuOpen && (
            ...
            >
                <div className="flex flex-col gap-6">
                    <button onClick={() => handleMobileNav('scan')} className="text-2xl font-bold text-white text-left flex items-center gap-4">
                    ...
                    <button onClick={() => handleMobileNav('roast')} className="text-2xl font-bold text-white text-left flex items-center gap-4">
                    ...
                    <button onClick={() => handleMobileNav('pricing')} className="text-2xl font-bold text-zinc-400 text-left flex items-center gap-4">
                         <span className="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-500">P</span>
                        Pricing
                    </button>
                    <button onClick={() => handleMobileNav('blog')} className="text-2xl font-bold text-zinc-400 text-left flex items-center gap-4">
