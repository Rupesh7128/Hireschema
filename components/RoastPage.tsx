...
import { generateContent, extractTextFromPdf } from '../services/aiService';
...
const SocialShareButtons = ({ score }: { score: number }) => {
    const [copied, setCopied] = useState(false);
    const shareUrl = window.location.origin + '/roast-my-resume';
    const shareText = `I just got my resume roasted by AI 🔥 Score: ${score}/100. It hurts but it's true. Try yours:`;

    const handleShare = (platform: string) => {
        logEvent('roast_share', { method: platform, score });
        if (platform === 'twitter') {
    ...
    return (
        <div className="flex flex-wrap justify-center gap-3 mt-4 mb-8">
            <button onClick={() => handleShare('twitter')} className="flex items-center gap-2 px-4 py-2 bg-[#1DA1F2]/10 text-[#1DA1F2] hover:bg-[#1DA1F2]/20 rounded-lg font-bold text-xs uppercase tracking-wider transition-colors">
                <Twitter className="w-4 h-4" /> Share on X
            </button>
            <button onClick={() => handleShare('linkedin')} className="flex items-center gap-2 px-4 py-2 bg-[#0A66C2]/10 text-[#0A66C2] hover:bg-[#0A66C2]/20 rounded-lg font-bold text-xs uppercase tracking-wider transition-colors">
                <Linkedin className="w-4 h-4" /> Share on LinkedIn
            </button>
            <button onClick={() => handleShare('copy')} className="flex items-center gap-2 px-4 py-2 bg-zinc-800 text-zinc-400 hover:bg-zinc-700 rounded-lg font-bold text-xs uppercase tracking-wider transition-colors">
                        ...
                        ">
                            ...
                            <div className="flex flex-col items-center my-8">
                                <ShareCard
                                    ...
                                    verdict={
                                        roastScore < 25 ? "Unhireable" :
                                            roastScore < 40 ? "Questionable" :
                                                roastScore < 55 ? "Mediocre" :
                                                    roastScore < 70 ? "Decent" : "Hireable"
                                    }
                                />
                                <SocialShareButtons score={roastScore} />
                            </div>
